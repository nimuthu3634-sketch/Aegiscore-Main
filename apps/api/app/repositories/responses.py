...
# Handles database operations related to response actions.
class ResponsesRepository:
    ...
    def normalize_ip_for_dedup(ip: str | None) -> str | None:
        # Normalizes IP addresses so duplicate block actions can be detected correctly.
    ...
    def find_existing_block_ip_for_alert_target(
        ...
        *,
        ...
        """Return an existing block_ip for this alert whose target matches ``target_ip``."""

        want = self.normalize_ip_for_dedup(target_ip)
        if want is None:
            return None

        # Checks already saved response actions in the database.
        ...
        # Also checks actions added in the current session before they are committed.
    ...
    def list_response_actions(
        ...
        # Builds the main response action list query used by the frontend.
        ...
        # Applies search filtering across response ID, action type, incident title, and details.
        if query.search:
        ...
        # Filters response actions by dry-run/live mode.
        ...
        # Converts frontend status filters into actual database status values.
        ...
        # Gives each status a rank so sorting by status is more meaningful.
        status_rank = case(
    ...
    def find_existing_automation_action(
        ...
        *,
        ...
        # Prevents creating the same automated response more than once.
        statement = select(ResponseAction).where(
        ...
        # Checks pending objects in the current session as well.
        added = getattr(self.session, "added", None)
        if isinstance(added, list):
            for obj in added:
                if not isinstance(obj, ResponseAction):
    ...
    def find_existing_policy_block_ip_for_alert(
        ...
        *,
        ...
        """Check whether a policy-based block_ip action already exists for this alert."""

        # This avoids creating a duplicate built-in ML block if a policy already blocked the IP.
        statement = (
            select(ResponseAction)
            .where(
                ResponseAction.normalized_alert_id == normalized_alert_id,
    ...
    def find_existing_policy_action(
        ...
        *,
        ...
        # Finds whether the same policy has already created an action for this incident/alert.
        statement = select(ResponseAction).where(
    ...
    def create(self, response_action: ResponseAction) -> ResponseAction:
        # Adds a new response action to the database session.
